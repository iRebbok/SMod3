...
namespace SMod3.Module.Piping
{
	public class PluginPipes
	{
		...
		public PluginPipes(Plugin plugin)
		{
			...
			{
				PipeFieldAttribute pipe = field.GetCustomAttribute<PipeFieldAttribute>();
				if (pipe != null)
				{
			...
			{
				PipePropertyAttribute pipe = property.GetCustomAttribute<PipePropertyAttribute>();
				if (pipe != null)
				{
			...
			foreach (MethodInfo method in pluginType.GetMethods(flags | BindingFlags.NonPublic))
			{
				if (method.IsPublic)
				{
					PipeMethodAttribute pipeMethod = method.GetCustomAttribute<PipeMethodAttribute>();
					if (pipeMethod != null)
					{
				...
				PipeEventAttribute pipeEvent = method.GetCustomAttribute<PipeEventAttribute>();
				if (pipeEvent != null)
				{
